...
export const ModalWTransition = ({classNameforBackgrounds, modalForCartilla, children, state, ChangeState, titulo='Alerta', background, showHeader, showOverlay, positionModal, padding, positionbtnclose, putWidth}) => {
      ...
      <>    
        ...
        <Overlay className={'modal-overlay'} showOverlay={showOverlay} positionModal={positionModal} ref={modalref} onClick={handleClick}>
            <ContenedorModal padding={padding} positionbtnclose={positionbtnclose} putWith={putWidth} className={classNameforBackgrounds? classNameforBackgrounds : 'bg-white'}>
                {showHeader &&
                <EncabezadoModal onMouseMove={overMouse}>
                    <h3>{titulo}</h3>
                </EncabezadoModal>  
                }
